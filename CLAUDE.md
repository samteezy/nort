...
On-demand: checking Force_Reprocess__c on a signature → NortSignatureTrigger (before update)
      → NortSignatureReprocessHandler [mark Queued · reset box] → NortDiagnosisQueueable.dispatch()
      (re-diagnoses past the cooldown; the trigger itself never invokes the agent)
```
...
- **Agent = iterative orchestrator, prompt template = research step:** the deployed `Nort_Error_Diagnosis_Agent` (Agent Script bundle in `aiAuthoringBundles/`) never reads source itself — it decides *which* code/metadata is implicated, passes `type:identifier` tokens to the `Nort_Error_Research` prompt template (`analyze_error`), reasons over its structured `findings`/`likelyCause`/`suggestedNextItems` (the `nortEvidence` `lightningTypes/` schema), and **loops** with refined tokens until confident or budget (`Analysis_Count < 10`) is spent — then composes the final `{rootCause, recommendation}`, quoting implicated code verbatim. The template's Apex grounding (`NortAnalysisGroundingProvider`) resolves the tokens and assembles ONE bounded context; the template is a research step, never the final verdict. Knowledge/platform questions are *direct* agent actions (standard Salesforce-documentation + record-query; `NortKnowledgeRetrievalAction` is a deployed-but-unwired MCP-ready seam). `NortAgentDiagnosisAction` is a separate Flow-callable `@InvocableMethod` over the same `NortAgentInvoker.diagnose` path (one already-deduped signature; not an agent tool).
- **Tooling API seam:** `NortToolingClient` is the single same-org org-introspection seam (Named Credential callout: `query`/`getRecord` + `fetchFlowMetadata`). It reaches what SOQL can't (flow internals, Apex symbol tables). Injectable via `setClient(...)`; the default `HttpToolingClient` is the only live integration and is mocked in tests (`Test.setMock` / injected client). Named Credential name + the spend caps come from `NortConfig`. Grounding is read-only — **no DML before its callouts**.
...
- Flow internal logic is unreachable via standard SOQL (`FlowDefinitionView` is metadata-only); `NortFlowMetadataAction` returns metadata + a note. Deeper internals (and Apex symbol tables, field definitions) are now retrieved via the **Tooling API** — `NortToolingClient` (same-org Named Credential callout) — and assembled by `NortAnalysisGroundingProvider`.
- Deployable metadata now covers all the code, the `Nort_Error_Research` prompt template (`genAiPromptTemplates/`, incl. its `apex://NortAnalysisGroundingProvider` grounding binding) + its `nortEvidence` output schema (`lightningTypes/`), the `Nort_Tooling` External + Named Credential, **and** the `Nort_Error_Diagnosis_Agent` Agent Script bundle (`aiAuthoringBundles/`). What still can't live in source: **publishing/activating** the deployed agent, the **External Client App** + its consumer key/secret, the credentials' org-specific URLs (Named Credential `Url`, External Credential `AuthProviderUrl`), the External-Credential **principal-access grant** on the permission set (the principal only exists once its secret is saved), and the inbound Email Service address — see `docs/AGENT_SETUP.md`.
