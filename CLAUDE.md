...
- **Agent = orchestrator, prompt template = analyzer:** the agent decides *which* code/metadata is implicated and passes a list of `type:identifier` tokens to the `Nort_Error_Analysis` prompt template (composed in Agent/Prompt Builder). The template's Apex grounding (`NortAnalysisGroundingProvider`) resolves those tokens, assembles ONE bounded context, and the template analyzes it. Knowledge retrieval stays a *direct* agent action — the template is strictly code/metadata analysis.
...
- Deployable metadata now covers all the code **and** the `Nort_Error_Analysis` prompt template (`genAiPromptTemplates/`, incl. its `apex://NortAnalysisGroundingProvider` grounding binding) **and** the valid `Nort_Tooling` External + Named Credential. What still can't live in source and is configured in Setup/Agent Builder: the Agentforce agent (topic + grounded prompt), the **External Client App** + its consumer key/secret, the credentials' org-specific URLs (Named Credential `Url`, External Credential `AuthProviderUrl`), the External-Credential **principal-access grant** on the permission set (the principal only exists once its secret is saved), and the inbound Email Service address — see `docs/AGENT_SETUP.md`.
