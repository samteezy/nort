...
- The Agentforce agent (topic + grounded prompt), the **`Nort_Error_Analysis` prompt template** (+ its model and grounding-resource binding), the **External Client App** behind the Tooling Named Credential, and the Email Service are configured in Setup/Agent Builder/Prompt Builder, not deployed — see `docs/AGENT_SETUP.md`. Deployable metadata covers all the code (incl. `NortToolingClient`/`NortAnalysisGroundingProvider`) and the Named/External Credential skeletons.
